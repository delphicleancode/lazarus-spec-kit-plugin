...
type
  { TQwenClient - HTTP client for Qwen/DashScope API (OpenAI-compatible endpoint) }
  TQwenClient = class(TInterfacedObject, ISpecAIClient)
  private
    FApiKey: string;
    FBaseURL: string;
    FTimeout: Integer;
...
const
  ...
  QWEN_DEFAULT_TIMEOUT = 120000; // 120 seconds

implementation

{ TQwenClient }

constructor TQwenClient.Create(const AApiKey: string);
begin
  inherited Create;
  FApiKey := AApiKey;
  FBaseURL := QWEN_DEFAULT_BASE_URL;
  FTimeout := QWEN_DEFAULT_TIMEOUT;
  FAborted := False;
  FHTTPClient := TFPHTTPClient.Create(nil);
end;

constructor TQwenClient.Create(const AApiKey, ABaseURL: string);
begin
  inherited Create;
  FApiKey := AApiKey;
  FBaseURL := ABaseURL;
...
destructor TQwenClient.Destroy;
begin
  FHTTPClient.Free;
  inherited Destroy;
end;

procedure TQwenClient.AbortRequest;
begin
  FAborted := True;
  // TFPHTTPClient has no mid-request cancel API; the flag prevents the next request
end;

function TQwenClient.BuildRequestJSON(const AMessages: TAIMessages;
  const AModel: string; AMaxTokens: Integer;
  ATemperature: Double): string;
var
...
function TQwenClient.ParseResponse(const AResponseBody: string): TAIResponse;
var
...
function TQwenClient.DoPost(const AEndpoint, ABody: string): string;
var
...
function TQwenClient.ChatCompletion(const AMessages: TAIMessages;
  const AModel: string; AMaxTokens: Integer;
  ATemperature: Double): TAIResponse;
var
  RequestBody: string;
...
begin
  if FApiKey = '' then
  begin
    Result := CreateErrorResponse('API key is not configured. Go to Settings to set your Qwen API key.');
    Exit;
  end;
...
function TQwenClient.TestConnection: Boolean;
var
...
begin
  ...
  Response := ChatCompletion(Messages, QWEN_DEFAULT_MODEL, 10, 0.0);
  Result := Response.Success;
end;

function TQwenClient.GetAvailableModels: TStringList;
begin
  Result := TStringList.Create;
  // DashScope does not provide a public models endpoint for listing; return hardcoded defaults
  Result.Add('qwen-turbo');
  Result.Add('qwen-plus');
  Result.Add('qwen-max');
  Result.Add('qwen-long');
  Result.Add('qwen-vl-max');
end;

function TQwenClient.GetProviderName: string;
begin
  Result := 'Qwen';
end;
